...
const userCart = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const cartTotal = Number(cartItem.price) * Number(cartItem.count);

    if (isNaN(cartTotal)) {
      return res.status(400).json({ status: "fail", message: "Invalid cart total" });
    }
...
const updateProductFromCart = asyncHandler(async (req, res) => {
  const { cartItemId, quantity } = req.body; // استقبلهم من البودي
...
const productIndex = userCart.products.findIndex(
  ...
  userCart.products[productIndex].count = quantity;

  // احسب التوتال من جديد لو محتاج
  userCart.cartTotal = userCart.products.reduce((acc, curr) => {
...
const createOrder = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // ✅ رجع رسالة الخطأ المفيدة للعميل
